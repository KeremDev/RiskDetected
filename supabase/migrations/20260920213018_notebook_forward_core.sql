-- Forward port for the existing staging schema. No rollout constraint replacement or schema-wide revocation.
-- Personal notebook remains fail-closed until the complete reminder delivery contract is verified.
CREATE FUNCTION public.isg_notebook_rollout_v1() RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $$
BEGIN
 PERFORM private_isg.active_actor();
 RETURN jsonb_build_object('schema_version',1,'enabled',coalesce((SELECT read_enabled AND write_enabled FROM private_isg.rollout WHERE feature='personal_notes'),false));
END $$;
REVOKE ALL ON FUNCTION public.isg_notebook_rollout_v1() FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.isg_notebook_rollout_v1() TO authenticated;
...
INSERT INTO private_isg.rollout(feature) VALUES('personal_notes') ON CONFLICT(feature) DO NOTHING;
...
REVOKE ALL ON private_isg.personal_notes,private_isg.note_conflicts,private_isg.note_items,private_isg.note_tags,private_isg.note_tag_links,private_isg.personal_reminders,private_isg.reminder_occurrences,private_isg.device_delivery_claims FROM PUBLIC,anon,authenticated,service_role;
