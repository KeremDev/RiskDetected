-- Google Play reports Android 2.0.2 (versionCode 14) live at 100%.
-- Advertise the live build while keeping both update prompts disabled.
do $migration$
declare
  v_policy jsonb;
...
begin
  ...
  if v_latest_build not in (13, 14) then
    raise exception 'Refusing to publish Android build 14 from unexpected latest build %', v_latest_build;
  end if;
  ...
  if v_latest_build = 14
    and (
      v_policy->>'policy_version' <> 'production-2.0.2-vc14'
      or coalesce((v_policy->>'soft_update_enabled')::boolean, true)
      or coalesce((v_policy->>'hard_update_enabled')::boolean, true)
    )
  then
    raise exception 'Unexpected Android build 14 policy state: %', v_policy;
  end if;
  ...
  foreach v_key in array array['analysis_engine_v4', 'analysis_result_hub_v1'] loop
    ...
    if v_value is null
      ...
      or not (coalesce(v_value->'enabled_android_builds', '[]'::jsonb) ?& array['7','8','9','10','11','12','13','14'])
    then
      raise exception 'Android build 14 release reconciliation failed for %', v_key;
    end if;
  end loop;
  ...
  foreach v_key in array v_runtime_keys loop
    ...
    then
      raise exception 'Android build 14 runtime reconciliation failed for %', v_key;
    end if;
  end loop;
...
begin
  ...
  if (v_policy->>'latest_build')::integer <> 14
    or coalesce((v_policy->>'soft_update_enabled')::boolean, true)
    or coalesce((v_policy->>'hard_update_enabled')::boolean, true)
    or v_policy->>'app_store_url' <> 'https://play.google.com/store/apps/details?id=com.riskdetectedan.app'
    or v_policy->>'policy_version' <> 'production-2.0.2-vc14'
  then
    raise exception 'Android build 14 release policy verification failed';
  end if;
end;
$verification$;
