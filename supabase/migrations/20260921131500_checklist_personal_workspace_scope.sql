-- Permit the checklist domain's explicitly personal runs inside an OSGB
-- workspace without weakening company-scoped writes in any other module.
BEGIN;
...
DECLARE workspace uuid:=private_isg.expert_workspace(); actor uuid; data jsonb:=to_jsonb(NEW);
 company uuid:=(data->>'company_id')::uuid; prior jsonb; patch jsonb; personal_scope boolean:=false;
BEGIN
 ...
 IF company IS NULL THEN
  CASE TG_TABLE_NAME
   ...
   WHEN 'checklist_runs' THEN
    personal_scope:=true;
    PERFORM private_isg.workspace_require_member(workspace,ARRAY['owner','admin','expert'],true);
   WHEN 'checklist_run_items' THEN
    SELECT company_id INTO company FROM private_isg.checklist_runs WHERE run_id=(data->>'run_id')::uuid;
    IF company IS NULL THEN
     personal_scope:=true;
     PERFORM private_isg.workspace_require_member(workspace,ARRAY['owner','admin','expert'],true);
    END IF;
 ...
 IF data ? 'owner_id' THEN patch:=patch||jsonb_build_object('owner_id',CASE WHEN personal_scope THEN actor ELSE NULL END); END IF;
