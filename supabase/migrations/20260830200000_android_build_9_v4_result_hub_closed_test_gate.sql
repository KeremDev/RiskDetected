-- RiskDetected Android 2.0.0 (versionCode 9) closed-test analysis gate.
--
-- Keep the release on the same V4 engine and result-hub contracts currently used by
-- approved iOS builds 87/88 and Android builds 7/8. Runtime submit gates already use
-- their minimum-version policy; this migration changes only the two explicit build
-- allowlists and does not advertise build 9 before Google Play processing completes.
...
begin
  foreach v_key in array v_runtime_keys loop
    ...
    if v_value is null
      ...
      or coalesce((v_value->>'min_android_version_code')::integer, 2147483647) > 9
      or coalesce((v_value->>'kill_switch')::boolean, true)
    then
      raise exception 'Android build 9 runtime gate preflight failed for %', v_key;
    end if;
  end loop;
  ...
  then
    raise exception 'Android build 9 V4 preflight state mismatch';
  end if;
  ...
  if v_value is null
    ...
    or not (coalesce(v_value->'enabled_ios_builds', '[]'::jsonb) ? '88')
  then
    raise exception 'Android build 9 result-hub preflight state mismatch';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Android build 9 active V4 configuration preflight failed';
  end if;
end;
$preflight$;
...
set value = jsonb_set(
      ...
      (
        ...
        from jsonb_array_elements_text(
          coalesce(value->'enabled_android_builds', '[]'::jsonb) || '["9"]'::jsonb
        ) as build
      ),
      true
    ),
...
begin
  ...
  then
    raise exception 'Android build 9 V4 gate verification failed';
  end if;
  ...
  then
    raise exception 'Android build 9 result-hub gate verification failed';
  end if;

  if coalesce((v_policy->>'latest_build')::integer, 0) >= 9 then
    raise exception 'Android build 9 must not be advertised before Play processing completes';
  end if;
end;
$verification$;
