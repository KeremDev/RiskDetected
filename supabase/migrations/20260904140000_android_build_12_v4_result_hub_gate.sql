-- RiskDetected Android 2.0.0 (versionCode 12) analysis gate.
--
-- Both analysis contracts are build allowlists, so a new Android build silently loses the V4
-- engine and the result hub until it is listed. Build 12 (dark-mode repairs, free-tier standard
-- report and the result photo viewer) joins iOS 87/88 and Android 7/8/9/10/11 on exactly the
-- contracts already in production; the runtime submit gates use minimum-version policy and need
-- no change. Build 12 is deliberately not advertised here — a publish_android_build_12 release
-- policy does that once Play reports it live.
do $preflight$
declare
  v_key text;
  v_value jsonb;
...
begin
  foreach v_key in array v_runtime_keys loop
    ...
    then
      raise exception 'Android build 12 runtime gate preflight failed for %', v_key;
    end if;
  end loop;
  ...
  if v_value is null
    ...
    or not (coalesce(v_value->'enabled_android_builds', '[]'::jsonb) ?& array['7', '8', '9', '10', '11'])
    or not (coalesce(v_value->'enabled_ios_builds', '[]'::jsonb) ?& array['87', '88'])
  then
    raise exception 'Android build 12 V4 preflight state mismatch';
  end if;
  ...
  if v_value is null
    ...
    or not (coalesce(v_value->'enabled_ios_builds', '[]'::jsonb) ?& array['87', '88'])
  then
    raise exception 'Android build 12 result-hub preflight state mismatch';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Android build 12 active V4 configuration preflight failed';
  end if;
end;
$preflight$;
...
set value = jsonb_set(
      ...
      (
        ...
        from jsonb_array_elements_text(
          coalesce(value->'enabled_android_builds', '[]'::jsonb) || '["12"]'::jsonb
        ) as build
      ),
      true
    ),
...
begin
  ...
  if not (coalesce(v_engine_flag->'enabled_android_builds', '[]'::jsonb) ?& array['7', '8', '9', '10', '11', '12'])
    or v_engine_flag->>'rollout_mode' <> 'build_allowlist'
    or coalesce(v_engine_flag->>'kill_switch', 'true') <> 'false'
  then
    raise exception 'Android build 12 V4 gate verification failed';
  end if;

  if not (coalesce(v_hub_flag->'enabled_android_builds', '[]'::jsonb) ?& array['7', '8', '9', '10', '11', '12'])
    or v_hub_flag->>'rollout_mode' <> 'build_allowlist'
    or coalesce(v_hub_flag->>'kill_switch', 'true') <> 'false'
  then
    raise exception 'Android build 12 result-hub gate verification failed';
  end if;

  if coalesce((v_policy->>'latest_build')::integer, 0) >= 12 then
    raise exception 'Android build 12 must not be advertised before Play goes live';
  end if;
end;
$verification$;
