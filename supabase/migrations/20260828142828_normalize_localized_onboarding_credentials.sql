...
begin
  ...
  if jsonb_typeof(v_raw_answers) is distinct from 'object' then
    raise exception 'invalid_raw_answers' using errcode = '22023';
  end if;

  v_app_language := lower(nullif(btrim(v_raw_answers->>'app_language'), ''));
  ...
  if p_certificate_class is not null
     and p_certificate_class not in ('A', 'B', 'C', 'doctor', 'otherHealth') then
    raise exception 'invalid_certificate_class' using errcode = '22023';
  end if;
  if not (v_hazard_classes <@ array['critical', 'high', 'low']::text[])
     or cardinality(v_hazard_classes) > 3 then
    raise exception 'invalid_hazard_classes' using errcode = '22023';
  end if;
  if not (v_sectors <@ v_allowed_sectors) or cardinality(v_sectors) > 15 then
    raise exception 'invalid_sectors' using errcode = '22023';
  end if;
  if p_audit_frequency is not null
     and p_audit_frequency not in ('1', '2-5', '6-15', '15+') then
    raise exception 'invalid_audit_frequency' using errcode = '22023';
  end if;
  if p_selected_plan is not null
     and p_selected_plan not in ('yearly', 'monthly') then
    raise exception 'invalid_selected_plan' using errcode = '22023';
  end if;
  ...
  values (
    v_user_id,
    'v2',
    case when v_is_english then null else p_certificate_class end,
    case when v_is_english then '{}'::text[] else v_hazard_classes end,
    v_sectors,
    p_audit_frequency,
    p_selected_plan,
    v_raw_answers,
  ...
  set
    ...
    certificate_class = case
      when v_is_english then null
      else coalesce(excluded.certificate_class, user_onboarding_answers.certificate_class)
    end,
    hazard_classes = case
      when v_is_english then '{}'::text[]
...
-- Older clients could leave Turkish credential fields attached to an English
-- terminology profile. Preserve the English role in raw_answers and remove
-- only the incompatible Turkish-only scalar/array fields.
