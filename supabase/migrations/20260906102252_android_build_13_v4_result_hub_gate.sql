...
begin
  foreach v_key in array v_runtime_keys loop
    ...
    then
      raise exception 'Android build 13 runtime gate preflight failed for %', v_key;
    end if;
  end loop;
  ...
  if not exists (
    ...
    raise exception 'Android build 13 active V4 configuration preflight failed';
  end if;
end;
$preflight$;
...
set value = jsonb_set(
      ...
      (
        ...
        from jsonb_array_elements_text(
          coalesce(value->'enabled_android_builds', '[]'::jsonb) || '["13"]'::jsonb
        ) as build
      ),
      true
    ),
...
begin
  ...
  if not (coalesce(v_engine_flag->'enabled_android_builds', '[]'::jsonb) ?& array['7', '8', '9', '10', '11', '12', '13'])
    or v_engine_flag->>'rollout_mode' <> 'build_allowlist'
    or coalesce(v_engine_flag->>'kill_switch', 'true') <> 'false'
  then
    raise exception 'Android build 13 V4 gate verification failed';
  end if;

  if not (coalesce(v_hub_flag->'enabled_android_builds', '[]'::jsonb) ?& array['7', '8', '9', '10', '11', '12', '13'])
    or v_hub_flag->>'rollout_mode' <> 'build_allowlist'
    or coalesce(v_hub_flag->>'kill_switch', 'true') <> 'false'
  then
    raise exception 'Android build 13 result-hub gate verification failed';
  end if;

  if coalesce((v_policy->>'latest_build')::integer, 0) >= 13 then
    raise exception 'Android build 13 must not be advertised before Play goes live';
  end if;
end;
$verification$;
