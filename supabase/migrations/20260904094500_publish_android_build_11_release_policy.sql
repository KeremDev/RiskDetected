-- Advertise Android 2.0.0 (versionCode 11) only after Google Play confirms the Production
-- release is live. The build-specific V4 and result-hub allowlists were opened by
-- 20260904093000; this migration advertises the live build while keeping both update prompts
-- disabled, exactly as build 10 shipped.
...
begin
  ...
  if v_latest_build > 11 then
    raise exception
      'Refusing to roll android_release_policy back from build % to 11',
      v_latest_build;
  end if;
  ...
  foreach v_key in array array['analysis_engine_v4', 'analysis_result_hub_v1'] loop
    ...
    then
      raise exception 'Android build 11 release preflight failed for %', v_key;
    end if;
  end loop;
  ...
  foreach v_key in array v_runtime_keys loop
    ...
    then
      raise exception 'Android build 11 runtime preflight failed for %', v_key;
    end if;
  end loop;
  ...
  set value = v_policy || jsonb_build_object(
        ...
        'policy_version', 'production-2.0.0-vc11'
      ),
      updated_at = now()
  where key = 'android_release_policy';
end;
...
begin
  ...
  if (v_policy->>'latest_build')::integer <> 11
    ...
    or v_policy->>'policy_version' <> 'production-2.0.0-vc11'
  then
    raise exception 'Android build 11 release policy verification failed';
  end if;
end;
$verification$;
