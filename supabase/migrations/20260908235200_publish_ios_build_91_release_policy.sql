-- iOS 2.0.3 (build 91) is publicly available on the App Store.
-- Admit the public build to both production analysis surfaces before advertising it
-- as the latest build, preventing build 91 from silently falling back to legacy flows.
do $migration$
declare
  v_policy jsonb;
...
begin
  ...
  if v_latest_build not in (90, 91) then
    raise exception 'Refusing to publish build 91 from unexpected iOS latest build %', v_latest_build;
  end if;

  if v_latest_build = 90
    and (
      v_policy->>'minimum_supported_build' <> '88'
      or v_policy->>'hard_update_enabled' <> 'true'
      or v_policy->>'soft_update_enabled' <> 'true'
      or v_policy->>'policy_version' <> 'build-90-appstore-general-release'
    )
  then
    raise exception 'Refusing to publish build 91 from unexpected build-90 policy state: %', v_policy;
  end if;

  if v_latest_build = 91
    and (
      v_policy->>'minimum_supported_build' <> '88'
      or v_policy->>'hard_update_enabled' <> 'true'
      or v_policy->>'soft_update_enabled' <> 'true'
      or v_policy->>'policy_version' <> 'build-91-appstore-general-release'
    )
  then
    raise exception 'Refusing to overwrite unexpected build-91 policy state: %', v_policy;
  end if;
  ...
  if v_engine_flag is null
    ...
    or not (coalesce(v_engine_flag->'enabled_ios_builds', '[]'::jsonb) ?& array['87', '88', '89', '90'])
  then
    raise exception 'iOS build 91 V4 preflight state mismatch';
  end if;
  ...
  if v_hub_flag is null
    ...
    or not (coalesce(v_hub_flag->'enabled_ios_builds', '[]'::jsonb) ?& array['87', '88', '89', '90'])
  then
    raise exception 'iOS build 91 result-hub preflight state mismatch';
  end if;
  ...
  if not exists (
    ...
    raise exception 'iOS build 91 active V4 configuration preflight failed';
  end if;
  ...
  set value = jsonb_set(
        ...
        (
          ...
          from jsonb_array_elements_text(
            coalesce(value->'enabled_ios_builds', '[]'::jsonb) || '["91"]'::jsonb
          ) as build
        ),
        true
      ),
  ...
  if not (coalesce(v_engine_flag->'enabled_ios_builds', '[]'::jsonb) ? '91')
    or not (coalesce(v_hub_flag->'enabled_ios_builds', '[]'::jsonb) ? '91')
  then
    raise exception 'iOS build 91 release gates are not ready';
  end if;
...
begin
  ...
  if v_policy->>'minimum_supported_build' <> '88'
    or v_policy->>'latest_build' <> '91'
    or v_policy->>'hard_update_enabled' <> 'true'
    or v_policy->>'soft_update_enabled' <> 'true'
    or v_policy->>'policy_version' <> 'build-91-appstore-general-release'
    or v_policy->>'app_store_url' <> 'https://apps.apple.com/tr/app/riskdetected-risk-assessment/id6769498181'
    or not (coalesce(v_engine_flag->'enabled_ios_builds', '[]'::jsonb) ? '91')
    or not (coalesce(v_hub_flag->'enabled_ios_builds', '[]'::jsonb) ? '91')
  then
    raise exception 'iOS build 91 release policy verification failed';
  end if;
end;
$verification$;
