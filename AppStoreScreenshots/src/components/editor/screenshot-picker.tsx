...
import { DEFAULT_LOCALE, resolveScreenshot } from "@/lib/locale";
...
export function ScreenshotPicker({ label, value, locale = DEFAULT_LOCALE, onChange }: Props) {
  ...
  const previewPath = hasValue && !isData ? resolveScreenshot(value, locale) : "";
  const previewSrc = isData ? value : previewPath ? img(previewPath) : "";
  // Only flag "image not found" when the path is a real URL that we tried and failed.
  const knownMissing = hasValue && !isData && didFail(previewPath);
  ...
  return (
    <div className="space-y-1">
      ...
      {error ? (
        ...
        <p className="text-[11px] text-destructive">Image not found at {previewPath || value}</p>
      ) : null}
    </div>
  );
}
