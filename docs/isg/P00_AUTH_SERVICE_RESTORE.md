...
Storage API/signed download, eski kullanıcıyla OTP/OAuth/parola giriş kanıtı, eski refresh/access token ve provider-secret taşınabilirliği, e-posta teslimi, gerçek native SDK/auth callback, iki platformda yerinde update, Realtime/Edge/external servis config restore. Test ortamında autoconfirm yalnız sentetik admin testine hizmet eder; production Auth ayarı değiştirilmedi. JWT logout erişim token'ını anında kriptografik olarak geçersiz kılmış sayılmaz; hassas mutation için session denetimi P01/P02'de ayrıca gerekir.
