...
| P12 Bildirim | Kısmi; omurga + gönderim güvenliği yerel olarak tamamlandı | Dört amaç, rıza kökeni, üretici sahipliği/shadow. **Devir sonrası:** gerçek zaman ve vade kontrolü, tekil tokenlı gönderim hakkı, paralel kapasite rezervasyonu, güncel izinle retry, backoff ve belirsiz sonuçta otomatik resend yasağı; 38 yeni kabul  APNs/FCM tek istekli adaptör ve işçi portları da eklendi; 53 ağsız davranış testi geçti; gerçek DB/credential bağlaması açık. | Gerçek APNs/FCM/e-posta adaptörleri, kısıtlı işçi ve token yönetimi, P01 tüketicisi, onboarding/native izin yüzeyi, schedule güncelliği, belirsiz sonuç mutabakatı, simulate/canary ve onaylı politika, teslim/okundu metrikleri, gerçek cutover |
| P13 Kişisel not/reminder | Kısmi; sunucu tarafı yerel olarak tamamlandı | **14 Eylül:** firma domaininden bağımsız Free defter, iki metni koruyan çakışma çözümü, diriltilemeyen tombstone, seriden bağımsız occurrence, DST'de sabit yerel saat, hatırlatıcı başına tek teslim sahibi | İstemci senkron motoru ve çevrimdışı kuyruk, kurtarma ekranı, cihaz tarafı DST/reboot/Focus/exact-alarm kabulleri, etiket fonksiyonları, retention |
