...
> **Güncel sunucu dilimi:** [Periyodik Kontroller](P10_EQUIPMENT_CHECKS_2026-09-15.md) — `20260915030000_isg_equipment_checks.sql` + `20260915050000_isg_equipment_periods.sql`. **İkinci migration sahibin kararıyla ilk dilimin bir kararını tersine çevirdi:** artık her tür bir varsayılan süreyle geliyor (`regulation_default`, gerekçe notu zorunlu, onay bayrağı şemayla zorunlu, uzman bu etiketi kendisi seçemez). Sonraki tarih rapordan otomatik hesaplanıp forma doluyor ve uzman değiştirebiliyor; `due_source` kaydedilen tarihin kimin cevabı olduğunu söylüyor. Kontrol kaydında opsiyonel İSG-KATİP beyanı var ve `katip_official_verification` yalnız false olabiliyor. Sentetik koşu **1298/0**, 50'si bu modülün. İlk dilim: P10'un ekipman modülüne istemci sınırı ve — domain fonksiyonlarında hiç olmayan — **sahiplik kontrolü** eklendi. `public.isg_equipment_checks_read_v1` / `..._mutate_v1`; her yazma kuralı elinde tutan P10 fonksiyonuna devrediliyor, bu dilim tarih hesaplamıyor. Tür için süre yoksa `period_unknown` (asla `valid`), olumsuz sonuç da tarih üretmiyor, tür öneri kataloğunda period kolonu yok, sonradan tanımlanan süre eski raporu yeniden yazmıyor. Altı durum okuma anında hesaplanıyor; beş sayaç ve aynı kelimelerle filtre. Menü + firma sayfası + **ana sayfa özet kartı** + Diğer Dosyalar arşiviyle rapor seçimi bağlandı. Sentetik koşu **1287/0**, 39'u bu dilimin. **Kendi rollout satırı yok; açmak `modules` ve `equipment` anahtarlarının ikisini birden ister.** Onaylanmış süre kataloğu yok, bu yüzden hiçbir tür için hazır süre sunulmuyor.
