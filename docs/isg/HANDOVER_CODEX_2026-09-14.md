...
> **Güncel sunucu dilimi:** [Periyodik Kontroller](P10_EQUIPMENT_CHECKS_2026-09-15.md) — `20260915030000_isg_equipment_checks.sql` + `20260915050000_isg_equipment_periods.sql` + `20260915070000_isg_equipment_inspection_edit.sql`. Kaydedilmiş rapor düzeltilebiliyor; **kontrol tarihi ve sonucu düzeltme allowlist'inde yok** (raporun kendisi onlar). İSG-KATİP işareti **yalnızca bilgi amaçlı**: hiçbir duruma, sayaca veya filtreye dokunmuyor ve `katip_official_verification` yalnız false olabiliyor. **İkinci migration sahibin kararıyla ilk dilimin bir kararını tersine çevirdi:** artık her tür bir varsayılan süreyle geliyor (`regulation_default`, gerekçe notu zorunlu, onay bayrağı şemayla zorunlu, uzman bu etiketi kendisi seçemez). Sonraki tarih rapordan otomatik hesaplanıp forma doluyor ve uzman değiştirebiliyor; `due_source` kaydedilen tarihin kimin cevabı olduğunu söylüyor. Kontrol kaydında opsiyonel İSG-KATİP beyanı var ve `katip_official_verification` yalnız false olabiliyor. Sentetik koşu **1305/0**, 57'si bu modülün. İlk dilim: P10'un ekipman modülüne istemci sınırı ve — domain fonksiyonlarında hiç olmayan — **sahiplik kontrolü** eklendi. `public.isg_equipment_checks_read_v1` / `..._mutate_v1`; her yazma kuralı elinde tutan P10 fonksiyonuna devrediliyor, bu dilim tarih hesaplamıyor. Tür için süre yoksa `period_unknown` (asla `valid`), olumsuz sonuç da tarih üretmiyor, tür öneri kataloğunda period kolonu yok, sonradan tanımlanan süre eski raporu yeniden yazmıyor. Altı durum okuma anında hesaplanıyor; beş sayaç ve aynı kelimelerle filtre. Menü + firma sayfası + **ana sayfa özet kartı** + Diğer Dosyalar arşiviyle rapor seçimi bağlandı. Sentetik koşu **1287/0**, 39'u bu dilimin. **Kendi rollout satırı yok; açmak `modules` ve `equipment` anahtarlarının ikisini birden ister.** Onaylanmış süre kataloğu yok, bu yüzden hiçbir tür için hazır süre sunulmuyor.

> **Güncel sunucu dilimi:** [Diğer Dosyalar arşivi](P04_FILE_LIBRARY_2026-09-14.md) — `20260915010000_isg_file_library.sql`: P04'ün ilk diliminin bıraktığı dört eksik (bucket, storage policy, tarayıcı, istemci yüzeyi) eklendi. İki private bucket, istemciye karantinaya yalnız INSERT / arşivden yalnız SELECT; `isg-file-inspect` Edge Function'ı + `_shared/isg/file-format-inspector.ts` biçim denetçisi; `public.isg_file_library_read_v1` / `..._mutate_v1` sınırı ve üçüncü giriş olarak yalnız `service_role`'a açık `isg_file_inspection_v1`. Verdikt istemciden verilemez (giriş aktör argümanı almaz). Denetçi **antivirüs değildir** ve her satır bunu söyler. Durum saklanmaz, indirme yolu yalnız asset varken çıkar, promote içerik adreslidir. Menü + firma detay sayfası bağlandı. Sentetik koşu **1248/0**, 48'i bu dilimin; denetçi 37/37. **Rollout açılmadı; açmak `file_core` ve `file_library` anahtarlarının ikisini birden ister.** Dağıtılmış fonksiyon gerçek Storage'a karşı koşulmadı.

> **Güncel sunucu dilimi:** [Evrak Takibi portföyü](P11_DOCUMENT_TRACKING_2026-09-14.md) — `20260914230000_isg_document_portfolio.sql`: sayfa artık hesabın tamamını **tek toplulaştırmayla** açıyor (firma başına sorgu yok), firma ve durum filtreleri, ilk 10 kayıt + Daha fazla göster, kompakt kayıt popup'ı ve yerinde kopya kaydetme paneli. Firma detay sayfasındaki Risk / Acil Durum / Periyodik Kontroller / Kurul / Zimmet / Temsilci / Diğer Dosyalar başlıkları aynı takip kaydını `kind_counts` ile tek çağrıda okuyor. Sentetik koşu 1200/0, bunun 42'si bu dilimin. Rollout **açılmadı**.

> **Güncel sunucu dilimi:** [Evrak Takibi](P11_DOCUMENT_TRACKING_2026-09-14.md) — `20260914210000_isg_document_tracking.sql`: firma bazında evrak yükümlülüğü ve dosyadaki kopyalar, `public.isg_document_tracking_read_v1` / `..._mutate_v1` sınırı, NOVA ekranları ve menü bağlantısı. Durum saklanmıyor (okuma anında hesaplanıyor), sağlık evrakı şemada yok, dosya saklanmıyor, uygunluk kararı üretilmiyor. Sentetik koşu 1189/0. Rollout **açılmadı**. **Commit yalnız bu dilimi içerir:** çalışma ağacında eşzamanlı bir oturum eğitim modülünü yazıyor; `NovaPilotMainGate.swift`, `Localizable.xcstrings` ve bu devir notu ortak olduğu için bu dosyalarda sadece bu dilimin satırları commit edildi, eğitim çalışması ağaçta bırakıldı.

> **Canlı eğitim v2 / build 100:** [Gerçekleşmiş çok firmalı eğitim](NOVA_TRAINING_SESSIONS_2026-09-14.md). Planlama/yoklama adımları kaldırıldı; katalog, sınıfa göre otomatik süre/periyot, özel başlık, çok firmalı personel seçimi, düzenleme/silme geçmişi ve imzasız kayıt PDF'i eklendi. Ledger `20260914130700`. Eski kayıt korundu. **Ek-2 şablonunu kullanıcı sonra verecek; resmî sertifika ertelendi.**

> **Canlı eğitim pilotu:** [NOVA eğitim takibi](NOVA_TRAINING_PILOT_2026-09-14.md) — eğitim planı, firma personeli, yoklama, düzenleme/tamamlama/iptal ve kalıcı tekrar gönderim bağlandı. Canlı additive migration `20260914115818`; mevcut süreli P05 kapsamı korunuyor. Build 99 fiziksel iPhone'a kuruldu. Resmi P07 sertifika/mevzuat akışı değil; kullanıcı cihaz kabulü bekliyor.

> **Güncel native dilim:** [Analiz sonuç ekranları](P09_ANALYSIS_RESULT_REDESIGN_2026-09-14.md) — Analizlerim ve yeni Analiz Raporları sayfası (istatistik kartı + arama/filtre + kompakt kartlar); Analiz Sonucu sayfası kabuğun üstünde açılıyor, alt sekme çubuğu yerine sabit **Geri Dön / Rapor oluştur** barı; Risk Analizi'nde istatistik kartı, metot toggle'ı ve ayrı bulgu kartları; skorsuz bölümlerde bant gösterilmiyor; Onaylı Defter çizgili defter zemininde; bulgu detayı fotoğraf kapaklı popup ve `O × F × Ş` çarpanlı skor kartı. Aktarılan bant **uzmanın o an okuduğu metodun** bandı. Sunucu değişmedi, migration yok. **Üç eski kırık L10N kapısı onarıldı** (L10N-001/004/013). **L10N-018 katalog kilidi 699 Türkçe birim geride; yeniden mühürlemek sahibin kararı, bu turda yapılmadı.** Rollout **açılmadı**.
