...
| Simülatörde gerçek staging hesabıyla (`.osgbpilot`, 25.09.2026) | Sihirbaz (su kuyusu sondajı, 8 konu, 80 soru) Listelerim'e kaydedildi: 70 katalog sorusu ve 10 uzantı sorusu `copy_items` ile yöntem ve açıklamasıyla kopyalandı, kendi soru olarak yazılan yok. Aynı başlıkla tekrar kayıt ` (2)`, ` (3)` … ile ayrı liste açtı. **Kontrolü başlat** kaydedilen listeyi seçili getirip doğrudan kontrol ayrıntılarına geçti; başlatılan kontrolün ilk sorusu uzantıdan (`ACX-DRILLRIG-01`, yöntem `K`, açıklamalı) |
| Bu denemede bulunan iki hata | 1) Kayıt akışının kod ekranındaki altı kutu klavye odağını birbirinden kapıyor (saniyede ~150 değişim), hiçbir rakam girilemiyordu: tek gerçek alan + altı gösterim kutusu (`NovaOBCodeField`), UI testi `testVerificationCodeAcceptsTypedDigits`. 2) Sihirbazdan veya Listelerim'den **Kontrolü başlat**, başlatma ekranını önceki ekran kapanırken açıyordu: ilk yükleme iptal olup "Kontrol kapsamı yüklenemedi" gösteriyor, seçilen liste de başlatma ekranına ulaşmıyordu. Başlatma ekranı artık önceki ekran kapandıktan sonra ve listeyi taşıyan bir istekle açılıyor |
...
- [x] Gerçek staging hesabıyla Listelerim'e kaydetme, tekrar kaydetme (başlık çakışması) ve **Kontrolü başlat** geçişi (simülatör, iOS). Deneme staging hesabında altı "İSG saha kontrol listesi" ve bir açık kontrol bıraktı.
