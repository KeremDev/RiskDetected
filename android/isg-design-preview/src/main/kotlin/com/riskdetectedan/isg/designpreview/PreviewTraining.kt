...
internal object PreviewTrainingClient : NovaTrainingClient {
    ...
    private fun preset(hazard: String, minutes: Int) = NovaEducationPackage.Preset("initial-$hazard", "İlk temel eğitim · $hazard", "initial", hazard,
        packageTopics.associate { it.code to minutes }, NovaEducationPackage.Preset.G4(listOf(NovaEducationPackage.Preset.G4Topic("G4.1", "Kırma eleme tesisi riskleri", 60)), 60),
        packageTopics.size * minutes + 60)
    ...
    override val companies: suspend () -> List<NovaCompanyOption> = {
        listOf(NovaCompanyOption("c1", "Koza Altın A.Ş", "Maden · Çok tehlikeli", null), NovaCompanyOption("c2", "Ege Lojistik", "Depolama · Az tehlikeli", null),
            NovaCompanyOption("c3", "Anadolu Gıda", "Gıda · Az tehlikeli", null, "low"))
    }
    override val userName = "Kerem Kayalar"
    override suspend fun page(after: String?) = NovaTrainingService.Page(listOf(session), null, setOf("c1", "c2", "c3"))
    override suspend fun employees(company: String) = listOf(NovaPersonOption("$company-e1", "Mehmet Kaya", "Üretim", "Operatör"),
        NovaPersonOption("$company-e2", "Elif Şahin", "Kalite", "Kalite uzmanı"), NovaPersonOption("$company-e3", "Can Yıldız", "Üretim", "Vardiya amiri"))
