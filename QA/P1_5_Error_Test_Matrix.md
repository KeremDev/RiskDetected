...
| --- | --- | --- | --- | --- | --- |
| E01 | Free günlük limit dolu | Free demo ile aynı gün 3. analizi başlat | Foto/metin girişi ve başlatma akışı Pro yönlendirmesi verir; teknik hata göstermez | Edge 429 varsa `quotaExceeded`, app alert/paywall support id ile uyumlu | Bekliyor |
| E02 | Foto seçmeden analiz | Foto modunda foto yokken `Taramayı Başlat` | Galeri/kamera seçim akışı açılır veya limit doluysa upload alanı bloklanır | Lokal validation, backend çağrısı yok | Bekliyor |
| E03 | Metin çok kısa/boş | Metin modunda boş/kısa metinle başlat | Kullanıcı metin alanına yönlendirilir, ham hata yok | Backend çağrısı yok | Bekliyor |
| E04 | Oturum yok/expired | App session temizlenmişken analiz başlat | “Tekrar giriş yap” tipi mesaj | Supabase auth error normalize edilir | Bekliyor |
| E05 | Offline ağ | Simulator ağını kes veya Mac bağlantısını kapat | “Bağlantıyı kontrol et” mesajı, UI stuck kalmaz | `networkUnavailable`, support id app logda | Bekliyor |
