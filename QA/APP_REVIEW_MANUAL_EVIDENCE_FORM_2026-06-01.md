...
| Supabase leaked-password toggle decision | Codex | PASS | 2026-06-02 05:13 +03 | Current release decision: do not pursue this now. It is not being treated as an App Review blocker for this submission path. |
...
| Final evidence collector rerun | Codex | PASS | 2026-06-02 04:52 +03 | `node scripts/app_review_preflight_collect.mjs --output QA/App_Review_Preflight_Evidence_2026-06-02.md` completed with `34 PASS`, `2 WARN`, `13 HOLD`, `1 SKIP`, `0 FAIL`. Screenshot count/dimensions, screenshot visual QA evidence, AI-assisted analysis disclosure, subscription paywall disclosure, release simulation source gating, submission-day runbook coverage, Review Notes helper coverage, physical-device readiness evidence including the 04:30 CoreDevice refresh, release local-evidence hygiene, ASC China mainland availability, Supabase deployed-function gates, Supabase Auth baseline gates, Supabase advisors, and Supabase `db lint` all ran in the collector. Supabase production secret enumeration is skipped by release decision. App Privacy publish, App Store screenshot visual approval/upload, China mainland availability decision, Supabase leaked-password decision, Review Notes placeholders, ASC contact fields, and the physical-device auth/onboarding/purchase/fresh-report subgates remain explicit HOLDs. Re-run once more after contact/App Privacy/screenshot upload/China/Supabase/manual smoke gates are completed. |
