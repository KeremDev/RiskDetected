...
p=Path('supabase/migrations/20260914210030_isg_pilot_module_management.sql')
modules={'emergency_plan':('emergency_plan_versions','plan_id','require_emergency_company','read_emergency_plans','emergency_plan_row'), 'drill':('drill_records','drill_id','require_drill_company','read_drills','drill_row'),'ppe':('ppe_handovers','handover_id','require_ppe_company','read_ppe_handovers','ppe_handover_row'),'appointment':('appointments','appointment_id','require_appointment_company','read_appointments','appointment_row')}
s="SET LOCAL lock_timeout='5s';\n"
for mod,(table,key,gate,read,row) in modules.items():
 s+=f'''ALTER TABLE private_isg.{table} ADD COLUMN is_deleted boolean NOT NULL DEFAULT false;
CREATE VIEW private_isg.live_{table} WITH (security_invoker=true) AS SELECT * FROM private_isg.{table} WHERE NOT is_deleted;
REVOKE ALL ON private_isg.live_{table} FROM PUBLIC,anon,authenticated,service_role;
'''
# Rewrite reads only; parent references stay physical so historical relations survive.
s+="DO $rewrite$ DECLARE r record; definition text; BEGIN\n"
for mod,(table,key,gate,read,row) in modules.items():
 s+=f"FOR r IN SELECT p.oid FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='private_isg' AND p.proname IN ('{read}','{row}') LOOP definition:=pg_get_functiondef(r.oid); definition:=replace(definition,'private_isg.{table}','private_isg.live_{table}'); EXECUTE definition; END LOOP;\n"
s+='END $rewrite$;\n'
s+='''CREATE TABLE private_isg.module_record_links (
...
'''
for mod,(table,key,gate,read,row) in modules.items():
 order=' ORDER BY t.version DESC' if mod=='emergency_plan' else ''
 s+=f"IF p_module='{mod}' THEN SELECT to_jsonb(t) INTO result FROM private_isg.{table} t WHERE t.{key}=p_id AND t.company_id=p_company AND NOT t.is_deleted{order} LIMIT 1 FOR UPDATE; END IF;\n"
s+='''IF result IS NULL THEN RAISE EXCEPTION USING ERRCODE='P0001',MESSAGE='ACCESS_DENIED'; END IF;
...
IF p_action='link_document' THEN
 ...
 ON CONFLICT(module,record_id) DO UPDATE SET obligation_id=excluded.obligation_id;
...
'''
for mod,(table,key,*_) in modules.items():s+=f"IF module='{mod}' THEN UPDATE private_isg.{table} SET is_deleted=true WHERE {key}=id AND company_id=p_company; END IF;\n"
s+='''ELSE
...
'''
# Column/variable ambiguity must never choose a column implicitly.
s=s.replace('ON CONFLICT(module,record_id) DO UPDATE','ON CONFLICT ON CONSTRAINT module_record_links_pkey DO UPDATE')
s += """
...
END $$;
"""
for table in ['emergency_plan_versions','drill_records','ppe_handovers','appointments']:
 s+=f'CREATE TRIGGER guard_deleted_module_record BEFORE UPDATE ON private_isg.{table} FOR EACH ROW EXECUTE FUNCTION private_isg.guard_deleted_module_record();\n'
s += """
