...
export function probeP05Upgrade({sql,pass,isolatedCopy}) {
  ...
  pass('p05_full_copy_dispatch_and_quota_rollout_closed', sql("SELECT count(*)=5 AND bool_and(NOT read_enabled AND NOT write_enabled) FROM private_isg.rollout WHERE feature IN ('event_dispatch','quota_ledger','file_core','rule_engine','training');") === 't');
  ...
  pass('p05_full_copy_new_schema_rls', sql("SELECT count(*)=49 AND bool_and(rowsecurity) FROM pg_tables WHERE schemaname='private_isg';") === 't');
