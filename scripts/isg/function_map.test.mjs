...
test('all seventeen ISG transport sources map to reviewed harnesses without claiming release readiness', () => {
  const result = verifyFunctionMap(); assert.equal(result.ok, true); assert.equal(result.runtime_files, 17);
