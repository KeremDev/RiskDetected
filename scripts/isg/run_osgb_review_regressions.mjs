...
const root=new URL('../../',import.meta.url),container=`isgada_osgb_review_${process.pid}`;
...
try{
  ...
  const sql=[
    ...
    read('scripts/isg/osgb_review_preupgrade.sql'),
    'BEGIN;',
    ...
    read('scripts/isg/osgb_review_regression_check.sql'),
  ].join('\n');
  const output=docker(['exec','-i',container,'psql','-U','postgres','-X','-v','ON_ERROR_STOP=1'],sql);
  const evidence=output.split('\n').filter(line=>/NOTICE:.*ok review/.test(line)).join('\n');
  if(!evidence)throw new Error('integrated rehearsal produced no acceptance evidence');
  console.log(evidence);
  console.log('PASS: undeployed OSGB populated legacy and privacy regression chain in one disposable PostgreSQL');
}finally{
