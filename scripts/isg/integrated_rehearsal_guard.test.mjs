...
test('the rehearsal covers every gated feature this transition added',()=>{
  ...
  const migrations=readFileSync(resolve(ROOT,'supabase/migrations/20260915010000_isg_file_library.sql'),'utf8');
  ...
  assert.ok(listed.length>=18,`only ${listed.length} features are exercised`);
  assert.ok(declared.length>=18,'the probe drifted from the rollout feature list');
  // personnel is gated inside its own checked entry, so it is asserted on the row.
  assert.match(probe,/count\(\*\)=19 AND bool_and\(NOT read_enabled AND NOT write_enabled\) FROM private_isg\.rollout/);
});
