...
try {
 ...
 check('required contact validation and atomic failure',()=>{const before=sql('SELECT count(*) FROM public.companies;');for(const x of [{phone:null},{email:null},{phone:'abc'},{email:'bad'},{phone:'1234567890123456'}])fail(create({...x,m:randomUUID()}),'VALIDATION_ERROR');assert.equal(sql('SELECT count(*) FROM public.companies;'),before);});
