...
const suites = {
  ...
  'nova-design': ['--test', 'scripts/isg/nova_tokens.test.mjs', 'scripts/isg/nova_icons.test.mjs', 'scripts/isg/nova_session_host.test.mjs'],
};
const [suite, ...extra] = process.argv.slice(2);
if (!Object.hasOwn(suites, suite) || extra.length) {
  console.error('Usage: node scripts/isg/run_suite.mjs foundation|capacity-shadow|nova-design; only offline suites are enabled here.');
  process.exitCode = 1;
} else {
