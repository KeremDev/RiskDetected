...
const container = `isgada_osgb_workspace_${process.pid}`;
const read = path => readFileSync(new URL(path, root), 'utf8');
function docker(args, input) {
  const result = spawnSync('docker', args, {input, encoding: 'utf8', maxBuffer: 20 * 1024 * 1024});
...
try {
  ...
  const sql = [
    ...
    'COMMIT;',
    read('scripts/isg/osgb_workspace_check.sql'),
  ].join('\n');
  ...
  if (!evidence) throw new Error('workspace checks produced no acceptance evidence');
  console.log(evidence);
  console.log('PASS: undeployed OSGB workspace foundation in disposable PostgreSQL');
} finally {
