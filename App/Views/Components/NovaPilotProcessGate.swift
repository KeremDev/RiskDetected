...
struct NovaProcessEditor: View {
    ...
    @ViewBuilder private func siteVisitStep(_ catalogue: NovaProcessPage) -> some View {
        switch visitStep {
        case 0:
            NovaText(text: catalogue.workplaces.isEmpty ? RDLocalization.string("localizable.nova.pilot.process.gate.firma.kapsami.1c025ba2", table: .localizable, fallback: "Firma kapsamı") : RDLocalization.string("localizable.nova.pilot.process.gate.isyeri.4c52ea5a", table: .localizable, fallback: "İşyeri"), style: .sectionTitle)
            if catalogue.workplaces.isEmpty {
                NovaHelpHint(text: RDLocalization.string("localizable.nova.pilot.process.gate.ziyaret.secili.firma.kapsaminda.kaydedilecek.32c944a8", table: .localizable, fallback: "Ziyaret seçili firma kapsamında kaydedilecek."))
            } else {
        ...
        default:
            ...
            NovaCard(padding: 14) {
                VStack(alignment: .leading, spacing: 9) {
                    ...
                    visitReviewRow(catalogue.workplaces.isEmpty ? "Firma" : RDLocalization.string("localizable.nova.pilot.process.gate.isyeri.442f4fdc", table: .localizable, fallback: "İşyeri"), catalogue.workplaces.first(where: {
                        $0.id.uuidString.lowercased() == values["workplace_id"]?.text.lowercased()
                    })?.name ?? (catalogue.workplaces.isEmpty ? RDLocalization.string("localizable.nova.pilot.process.gate.firma.geneli.11c1eada", table: .localizable, fallback: "Firma geneli") : "—"))
