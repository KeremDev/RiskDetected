    ...
    private var photoArt: some View {
        ZStack {
            ...
            HStack(spacing: 6) {
                ...
                Text("Tehlike tespit edildi").font(.system(size: RDFontScale.size(11), weight: .semibold, design: .rounded))
            }
        ...
        ZStack(alignment: .bottomTrailing) {
            VStack(alignment: .leading, spacing: 8) {
                HStack {
                    RDLogo(size: 11)
                    Spacer()
                    Text("Rapor #2841").font(.system(size: RDFontScale.size(8), design: .rounded)).foregroundStyle(Color.rdSlate)
                }
